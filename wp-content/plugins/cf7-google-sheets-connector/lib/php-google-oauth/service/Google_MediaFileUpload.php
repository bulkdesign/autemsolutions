...
class Google_MediaFileUpload {
  ...
  public function nextChunk(Google_HttpRequest $req) {
    ...
    $response = CF7GSC_Google_Client::$io->authenticatedRequest($httpRequest);
  ...
  private function getResumeUri(Google_HttpRequest $httpRequest) {
    ...
    $response = CF7GSC_Google_Client::$io->makeRequest($httpRequest);
    $location = $response->getResponseHeader('location');
    $code = $response->getResponseHttpCode();
    if (200 == $code && true == $location) {
      return $location;
    }
    throw new Google_Exceptions("Failed to start the resumable upload");
  }
}