...
class Google_ServiceResource {
   ...
   * @throws Google_Exceptions
   */
  public function __call($name, $arguments) {
    if (! isset($this->methods[$name])) {
      throw new Google_Exceptions("Unknown function: {$this->serviceName}->{$this->resourceName}->{$name}()");
    }
    ...
    foreach ($parameters as $key => $val) {
      if ($key != 'postBody' && ! isset($method['parameters'][$key])) {
        throw new Google_Exceptions("($name) unknown parameter: '$key'");
      }
    }
    if (isset($method['parameters'])) {
      foreach ($method['parameters'] as $paramName => $paramSpec) {
        if (isset($paramSpec['required']) && $paramSpec['required'] && ! isset($parameters[$paramName])) {
          throw new Google_Exceptions("($name) missing required param: '$paramName'");
        }
    ...
    $httpRequest = CF7GSC_Google_Client::$auth->sign($httpRequest);
    if (CF7GSC_Google_Client::$useBatch) {
      return $httpRequest;
    }
